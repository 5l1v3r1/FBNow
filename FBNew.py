...
def subtitles():
    ...
    lang = input('Enter subtitles language ( english -> en ): ')
...
def song_download_by_name(name, sub, sub_data, url=''):
    ...
    command = ['youtube-dl', '-o', '/root/Music/%(title)s.%(ext)s+']
...
def video_song_download_by_name(name, sub, sub_data, url=''):
    ...
    command = ['youtube-dl', '-o', '/root/Videos/%(title)s.%(ext)s+']
    command.extend(search)
    command.extend(extras)
    call(command, shell=False)
...
def menu():
    print()
    print('1. Display soundcloud Weekly Top')
    print('2. Display worldwide Chartbusters')
...
def song_list_menu():
    ...
    print('1. soundcloud Weekly Top')
    print('2. worldwide Chartbusters')
...
def song_list_options(choice):
    ...
    if choice == '1':
        lang = input("Enter songs' lanuguage (english) : ")
        print()
        res = requests.get("https://soundcloud.com/featured/" + lang + "/Weekly+Top+Songs")
    
    if choice == '2':
        res = requests.get("https://soundcloud.com/charts")
